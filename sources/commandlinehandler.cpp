...
bool CommandLineHandler::validateJson(const QString &filePath, const QString &schemaPath) {
    ...
    jsonFile.read();
    qCInfo(CLASS_LC) << "Validating" << filePath << "with schema" << schemaPath;
    if (jsonFile.isValid()) {
        return true;
    } else {
        qCCritical(CLASS_LC).noquote() << "Invalid configuration!" << endl << jsonFile.error();
        return false;
    }
}
